...
import {
  View, Text, StyleSheet, ScrollView, FlatList, ActivityIndicator, StatusBar,
  Dimensions
} from 'react-native';
  ...
  useEffect(() => {
    ...
    const fetchOneWayMatches = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/getOneWayMatches`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });

        if (!response.ok) {
          console.error('Failed to fetch one way matches');
          setLoading(false);
          return;
        }

        const data = await response.json();
        setOneWayMatches(data.oneWayMatches || []);
      } catch (error) {
        console.error('Error fetching one way matches:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <View>
      <View style={styles.listContainer}>
        ...
        {profiles.length > 0 ? (
              ...
              <View style={styles.cardWrapper}>
                <ProfileCard
                  profile={{
                    ...
                    language: item?.mother_tongue_data?.mtongue_name || 'N/A', // ✅ FIXED LINE
          ...
          <Text style={styles.noProfilesText}>No One Way Matches Found</Text>
        )}
      </View>
    </View>
  );
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        <View style={styles.loaderContainer}>
          ...
          <Text style={styles.loaderText}>Loading one way matches...</Text>
        </View>
      ) : (
        <ScrollView style={{ marginBottom: height * 0.09 }}>
          {renderProfileList('ONE WAY MATCHES', oneWayMatches, handleProfileButtonPress)}
        </ScrollView>
      )}
