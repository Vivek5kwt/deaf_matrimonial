...
const Screen27 = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    const fetchTwoWayMatches = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/getTwoWayMatches`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });

        if (!response.ok) {
          console.error('Failed to fetch two-way matches');
          setLoading(false);
          return;
        }

        const data = await response.json();
        setOneWayMatches(data.twoWayMatches || []); // Assuming `twoWayMatches` is the correct key
      } catch (error) {
        console.error('Error fetching two-way matches:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const profileCounts = {
    'Two Way matches': oneWayMatches.length,
  };
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        <View style={styles.loaderContainer}>
          ...
          <Text style={styles.loaderText}>Loading Two way matches...</Text>
        </View>
      ) : (
        <ScrollView style={{ marginBottom: height * 0.09 }}>
          {renderProfileList('TWO WAY MATCHES', oneWayMatches, handleProfileButtonPress)}
        </ScrollView>
      )}
  ...
  <View>
    <View style={styles.listContainer}>
      ...
      {profiles.length > 0 ? (
        ...
        <Text style={styles.noProfilesText}>No Two Way Matches Found</Text>
      )}
    </View>
  </View>
);
